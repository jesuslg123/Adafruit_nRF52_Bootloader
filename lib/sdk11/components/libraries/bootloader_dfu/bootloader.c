...
static bool m_usb_was_mounted; /**< Tracks whether USB DFU was ever enumerated so later cable removal can exit startup DFU. */
static bool m_startup_dfu_has_activity; /**< Tracks whether startup DFU has received valid update traffic. */
...
//    APP_ERROR_CHECK(err_code);
    ...
    if (m_cancel_timeout_on_usb)
    {
      ...
      {
        // Only exit startup DFU after a real cable unplug, not on host-side re-enumeration.
        bootloader_timeout_startup_dfu();
      }
    }
#endif
